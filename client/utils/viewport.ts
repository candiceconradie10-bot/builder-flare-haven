...
function detectNotch(): boolean {
  ...
  const isIPhoneX = /iPhone.*OS 1[1-9]/.test(navigator.userAgent) && 
                   window.screen.height === 812 && window.screen.width === 375;
                   
  // Check for other devices with notches
  const hasDisplayCutout = 'CSS' in window && CSS.supports('top: env(safe-area-inset-top)');
...
export function setOptimalViewport(): void {
  ...
  let viewportContent = 'width=device-width, initial-scale=1.0, minimum-scale=1.0';
  ...
  inputs.forEach(input => {
    input.addEventListener('focus', () => {
      if (getViewportInfo().isMobile) {
        ...
        viewport.content = originalContent.replace(/maximum-scale=[^,]*/, 'maximum-scale=1.0');
...
export function initializeViewportOptimizations(): void {
  ...
  document.body.classList.add(
    viewportInfo.isMobile ? 'device-mobile' : 
    viewportInfo.isTablet ? 'device-tablet' : 'device-desktop'
  );
...
export function getResponsiveImageSize(baseWidth: number): {
  ...
  if (viewportInfo.isMobile) {
    width = Math.min(baseWidth, viewportInfo.width * viewportInfo.devicePixelRatio);
    sizes = '100vw';
  } else if (viewportInfo.isTablet) {
    width = Math.min(baseWidth, viewportInfo.width * 0.8 * viewportInfo.devicePixelRatio);
    sizes = '80vw';
  } else {
    width = Math.min(baseWidth, viewportInfo.width * 0.5 * viewportInfo.devicePixelRatio);
    sizes = '50vw';
  }
